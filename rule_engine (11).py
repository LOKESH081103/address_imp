...
def layer1_structural(addr: str, tokens, pins, min_words: int = 5, merge_len_threshold: int = 15):
    ...
    ) or _has_leading_zero_house_number(addr):
...
def _has_leading_zero_house_number(addr: str) -> bool:

    """

    True if the address literally starts with an all-zero number as its
    first token - e.g. "00 Malviya Nagar...", "0 Balaghat Road...". There is
    no such thing as house/plot number zero, so this is exactly as much a
    placeholder as "# 0" or "H.No 0" is - it's just written without the
    label. Only matches at the very start (the house-number position);
    a "0" appearing later in the address is left alone.
    """

    return bool(re.match(r"\s*#?\s*0+(?=[\s,.\-]|$)", addr))
...
def extract_state(addr_upper: str) -> str:
    """
    Best-effort match of which Indian state (from INDIAN_STATES) is
    mentioned in the address, for state-wise reporting.

    Returns the state name in Title Case, or "Unknown" if none of the
    recognized state names appear in the text - this is common for
    addresses that only name a well-known city (e.g. "Mumbai") without
    spelling out the state, or ones flagged STATE_NOT_FOUND in layer1.

    If more than one state name appears (rare - e.g. a pincode-mismatch
    address that mentions two states), the longest match wins, since
    that's usually the more specific/deliberate one.
    """
    matches = [s for s in INDIAN_STATES if s in addr_upper]
    if not matches:
        return "Unknown"
