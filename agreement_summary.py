...
follow-up, even if the other 4 are perfectly clean.

Severity is two-tier now (Critical / Clean - no "Warning" tier; see
rule_engine.severity_for). Rollup precedence (worst wins): Critical > Clean.
...
_RANK = {"Critical": 1, "Clean": 0, "Clean (reviewer-cleared)": 0}
_RANK_TO_SEVERITY = {1: "Critical", 0: "Clean"}

ROLLUP_COLUMNS = [
    "Agreement No", "Agreement Severity", "Address Count",
    "Critical Addresses", "Clean Addresses",
]
...
def agreement_rollup(result_df: pd.DataFrame, agreement_col: str = "Agreement No") -> pd.DataFrame:
    ...
    it has on file, and the Critical/Clean split among them.
    """
...
def summarize_agreements(result_df: pd.DataFrame, agreement_col: str = "Agreement No") -> dict:
    ...
    The headline counts for the summary cards:
      ...
      - clean/critical agreement counts, and how many addresses each of
        those buckets contains in total.
