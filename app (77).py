...
if df is not None:
    ...
    with c3:
        ...
        branch_col_choice = st.selectbox(
            ...
            help=f"Matched against the company's zone master "
                 ...
                 "zone-wise reporting - state-wise still works either way, straight from the address text.",
        )
    ...
    if st.button("🔍 Run address check", type="primary"):
        ...
        # ---------------- Geography: State (always) + Zone (if a branch column was picked) ----------------
    ...
    if "run_results" in st.session_state:
        ...
        zone_available = rr["zone_available"]
        branch_col = rr["branch_col"]
        ...
        st.caption(
            "Which Indian state each address mentions, matched from the address text itself - no extra "
            "column needed. Addresses that don't spell out a recognizable state name land in \"Unknown.\""
        )
