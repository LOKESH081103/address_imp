...
from geo_summary import breakdown_by_group
from geo_summary_image import build_geo_breakdown_png
import zone_mapping
...
if df is not None:
    ...
    with c3:
        branch_options = ["— none —"] + cols
        guessed_branch = guess(cols, "branch")
        default_branch_idx = branch_options.index(guessed_branch) if "branch" in str(guessed_branch).lower() else 0
        branch_col_choice = st.selectbox(
            "Branch column (optional, for zone-wise breakdown)", branch_options, index=default_branch_idx,
            help=f"Matched against the company's zone master "
                 f"({zone_mapping.known_branch_count():,} branches known). Leave as '— none —' to skip "
                 "zone-wise reporting - state-wise still works either way.",
        )
        branch_col = None if branch_col_choice == "— none —" else branch_col_choice
    with c4:
        state_options = ["— derive from address text —"] + cols
        guessed_state = guess(cols, "state")
        default_state_idx = state_options.index(guessed_state) if "state" in str(guessed_state).lower() else 0
        state_col_choice = st.selectbox(
            "State column (optional, for state-wise breakdown)", state_options, index=default_state_idx,
            help="If your file already has a State column, pick it here to use it as-is. Otherwise the app "
                 "guesses the state by matching known state names inside the address text.",
        )
        state_col = None if state_col_choice == "— derive from address text —" else state_col_choice
    ...
    if st.button("🔍 Run address check", type="primary"):
            ...
            if not ok:
                ...
                for i, row in result_df.iterrows():
                    ...
                    if is_human_cleared:
                        ...
                        new_severity_col.append(severity_for(issues, ml_prob=prob))
        ...
        # ---------------- Geography: State (own column, or derived) + Zone (if branch mapped) ----------------
        if state_col is not None:
            # Row order out of process_dataframe matches the uploaded file exactly (see its
            # docstring - phase 1 builds `rows` by iterating df[address_col] in order), so it's
            # safe to align this column back on by position rather than by any shared key.
            state_values = df[state_col].reset_index(drop=True).fillna("Unknown").astype(str).str.strip()
            state_values = state_values.replace({"": "Unknown", "nan": "Unknown", "None": "Unknown", "NaT": "Unknown"})
            result_df["State"] = state_values.str.title()
        else:
            result_df["State"] = result_df[address_col].astype(str).str.upper().map(extract_state)

        zone_available = branch_col is not None and zone_mapping.is_available()
        if zone_available:
            result_df = zone_mapping.add_zone_columns(
                result_df, branch_col, levels=("Zone", "Sub Zone", "Main Region", "Sub Region", "Area")
            )
    ...
    if "run_results" in st.session_state:
        ...
        comparison = rr.get("comparison")
        net_error_count = rr.get("net_error_count", 0)
        layer2_enabled = rr.get("layer2_enabled", False)
        use_ml = rr.get("use_ml", False)
        ml_algorithm = rr.get("ml_algorithm")
        ml_auto_select = rr.get("ml_auto_select", True)
        training_df = rr.get("training_df")
        model = rr.get("model")
        source_name = rr.get("source_name", "uploaded file")
        zone_available = rr.get("zone_available", False)
        branch_col = rr.get("branch_col")
        state_col = rr.get("state_col")

        if "run_results" in st.session_state and set(rr.keys()) < {
            "result_df", "comparison", "net_error_count", "layer2_enabled", "use_ml", "ml_algorithm",
            "ml_auto_select", "training_df", "model", "source_name", "zone_available", "branch_col", "state_col",
        }:
            st.info(
                "ℹ️ This result was computed with an older version of the app (some newer fields, like "
                "state/zone breakdown, weren't recorded then) - defaults were used below. Click "
                "'🔍 Run address check' again to refresh everything with the current version.",
                icon="ℹ️",
            )
        ...
        flagged_df = result_df[result_df["Severity"] == "Critical"].reset_index(drop=True)
        ...
        with st.expander("📋 Agreement-level detail table", expanded=False):
            sev_filter = st.multiselect(
                "Filter by agreement severity", ["Critical", "Clean"],
                default=["Critical"], key="agr_sev_filter",
            )
            shown = rollup_df[rollup_df["Agreement Severity"].isin(sev_filter)] if sev_filter else rollup_df
            st.dataframe(
                shown.sort_values(
                    by=["Agreement Severity", "Critical Addresses"],
                    key=lambda col: col.map({"Critical": 0, "Clean": 1}) if col.name == "Agreement Severity" else col,
        ...
        # ---------------- State-wise breakdown (always available - straight from the address text) ----------------
        st.subheader("3b. State-wise breakdown")
        if state_col is not None:
            st.caption(f"Using your **{state_col}** column directly, as-is (titlecased for consistent grouping).")
        else:
            st.caption(
                "Derived from the address text - matched against known Indian state names. Addresses "
                "that don't spell out a recognizable state name land in \"Unknown\". Pick a State column "
                "in step 2 if your file already has one, to use it directly instead."
            )
        state_bd = breakdown_by_group(result_df, "State")
        st.dataframe(state_bd, use_container_width=True, hide_index=True)

        state_png = build_geo_breakdown_png(state_bd, "State", source_name=source_name)
        st.image(state_png, caption="Preview - this is exactly what the download looks like", width=650)
        st.download_button(
            "🖼️ Download state-wise breakdown (.png)", data=state_png,
            file_name="state_wise_breakdown.png", mime="image/png", key="dl_state_png",
        )
        st.download_button(
            "⬇ Download state-wise breakdown (.xlsx)", data=to_excel_bytes_single(state_bd, "State Breakdown"),
            file_name="state_wise_breakdown.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="dl_state_xlsx",
        )

        # ---------------- Zone-wise breakdown (optional - needs a matched Branch column) ----------------
        st.subheader("3c. Zone-wise breakdown")
        if not zone_available:
            st.info(
                "No Branch column was selected (or none of its values matched the company's zone master) in "
                "step 2, so zone-wise reporting is off for this run. Pick your branch column there and re-run "
                f"to enable it - {zone_mapping.known_branch_count():,} branches are recognized."
            )
        else:
            matched = int((result_df["Zone"] != "Unknown").sum())
            unmatched = len(result_df) - matched
            if unmatched:
                st.caption(
                    f"⚠️ {unmatched:,} of {len(result_df):,} rows had a branch value not found in the zone "
                    f"master (typos, new/closed branches, etc.) - they're grouped as \"Unknown\" below rather "
                    "than dropped."
                )
            level = st.selectbox(
                "Break down by", ["Zone", "Sub Zone", "Main Region", "Sub Region", "Area"],
                key="zone_level_select",
            )
            zone_bd = breakdown_by_group(result_df, level)
            st.dataframe(zone_bd, use_container_width=True, hide_index=True)

            zone_png = build_geo_breakdown_png(zone_bd, level, source_name=source_name)
            st.image(zone_png, caption="Preview - this is exactly what the download looks like", width=650)
            st.download_button(
                f"🖼️ Download {level.lower()}-wise breakdown (.png)", data=zone_png,
                file_name=f"{level.lower().replace(' ', '_')}_wise_breakdown.png", mime="image/png",
                key="dl_zone_png",
            )
            st.download_button(
                f"⬇ Download {level.lower()}-wise breakdown (.xlsx)",
                data=to_excel_bytes_single(zone_bd, f"{level} Breakdown"),
                file_name=f"{level.lower().replace(' ', '_')}_wise_breakdown.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="dl_zone_xlsx",
            )
        ...
        with dash_c1:
            ...
            ).value_counts().reindex(["Critical", "Clean"]).fillna(0)
