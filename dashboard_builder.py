...
def summarize_from_result_df(result_df: pd.DataFrame, model_comparison: Optional[dict] = None,
    ...
    return {
        ...
        "ground_truth": None,
        "confusion": None,
        "severity": {"critical": critical, "clean": clean},
        "rule_flagged": critical,
...
new Chart(document.getElementById('sevChart'), {{
  type: 'doughnut',
  data: {{ labels: ['Critical','Clean'],
    datasets: [{{ data: {sev_data}, backgroundColor: ['#e34948','#2a78d6'], borderWidth: 2, borderColor: ring }}] }},
...
new Chart(document.getElementById('modelChart'), {{
  ...
  options: {{ ...common, plugins: {{ legend: {{ display: false }} }},
    ...
    return HTML_TEMPLATE.format(
        ...
        sev_data=json.dumps([sev["critical"], sev["clean"]]),
